...
my $msg = undef;
$tasks = $client->new_task_set;
$tasks->add_task('fail_die', undef, { on_exception => sub { $msg = shift }, });
$tasks->wait;
...
TODO: {
    ...
    is($client->do_task('sleep', 5, { timeout => 3 }),
        undef, 'Job that timed out after 3 seconds returns failure');
}
...
isnt($js_status->{'127.0.0.1:9050'}->{echo_prefix}->{capable},
    0, 'Correct capable jobs for echo_prefix');
isnt($js_status->{'127.0.0.1:9051'}->{echo_prefix}->{capable},
    0, 'Correct capable jobs for echo_prefix, again');
isnt($js_status->{'127.0.0.1:9052'}->{echo_prefix}->{capable},
    0, 'Correct capable jobs for echo_prefix, yet again');
is($js_status->{'127.0.0.1:9050'}->{echo_prefix}->{running},
    0, 'Correct running jobs for echo_prefix');
is($js_status->{'127.0.0.1:9051'}->{echo_prefix}->{running},
    0, 'Correct running jobs for echo_prefix, again');
is($js_status->{'127.0.0.1:9052'}->{echo_prefix}->{running},
    0, 'Correct running jobs for echo_prefix, yet again');
is($js_status->{'127.0.0.1:9050'}->{echo_prefix}->{queued},
    0, 'Correct queued jobs for echo_prefix');
is($js_status->{'127.0.0.1:9051'}->{echo_prefix}->{queued},
    0, 'Correct queued jobs for echo_prefix, again');
is($js_status->{'127.0.0.1:9052'}->{echo_prefix}->{queued},
    0, 'Correct queued jobs for echo_prefix, yet again');
...
$handle = $client->dispatch_background(
    long => undef,
    { on_complete => sub { $out = ${ $_[0] } }, }
);
